...
def _map_contract(c: dict, exp_ts: int) -> dict:
    ...
    # NANOSECONDS. Convert to unix seconds + an age in days so the frontend can
    # flag STALE / DEAD prices. A contract with no real trade (price 0 / no
    # timestamp) is marked hasRealTrade=False so the UI can treat it honestly.
    ...
    return {
        ...
        "hasRealTrade": has_real_trade,   # True only if a real last_trade.price came back
